...
export default function Page() {
  ...
  if (status === "error") return <p className="text-center">Error fetching achievements!</p>;
  ...
  return (
    <div className="w-full p-4">
        ...
        <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
          <thead className="bg-neutral-100 dark:bg-neutral-800">
            <tr>
              ...
              <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Achievement</th>
              ...
              <React.Fragment key={i}>
                {page?.data?.map((item, index) => (
                  <tr key={index} className="hover:bg-neutral-50 dark:hover:bg-neutral-800 transition">
                    ...
                    <td className="px-4 py-2 text-gray-800 dark:text-gray-200">{item?.achievement}</td>
      ...
      <div ref={loadMoreRef} className="w-full text-center mt-4">
        ...
        {!hasNextPage && <p className="text-gray-500">No more achievements</p>}
      </div>
    </div>
  );
}