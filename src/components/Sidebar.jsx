            ...
            {item?.categories?.length > 0 ? (
              <div className={`w-full ${toggle ? 'h-auto' : 'h-fit'} transform transition-height duration-800 ease-in-out`}>
                <button
                  ...
                  className="flex w-full items-center gap-2 p-2 hover:bg-red rounded cursor-pointer"
                >
                  {item.icon}
                  {item.title}
                </button>
                  ...
                  <ul className="ml-4 bg-gray-800 text-sm">
                    {item.categories.map(category => (
                      <Link 
                        href={`${category.path}`} 
                        key={category.id} 
                        className={`flex items-center gap-2 p-2 ${pathName === `${category.path}` ? 'active' : 'border border-gray-300'} rounded cursor-pointer`}
                      >
                        {category.icon}
              ...
              <Link 
                href={`${item.path}`} 
                className={`flex items-center gap-2 p-2 ${pathName === `${item.path}` ? 'active' : 'hover:border border-gray-300'} rounded cursor-pointer`}
              >
                {item.icon}
